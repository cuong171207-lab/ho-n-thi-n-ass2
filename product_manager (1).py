...
def update_product(products):
    ...
    for product in products:
        if product["id"] == product_id:
            ...
            product["brand"] = (
                input(f"Thương hiệu mới ({product['brand']}): ") or product["brand"]
            )
...
def delete_product(products):
    ...
    if found_product:
        print(
            f"Đã tìm thấy sản phẩm: {found_product['name']} (Giá: {found_product['price']})"
        )
...
def display_all_products(products):
    ...
    print("\n" + "=" * 85)
    print(
        f"{'MÃ':<10} | {'TÊN SẢN PHẨM':<30} | {'THƯƠNG HIỆU':<15} | {'GIÁ':<12} | {'SL':<5}"
    )
    print("-" * 85)
    ...
    for p in products:
        ...
        print(
            f"{p['id']:<10} | {p['name']:<30} | {p['brand']:<15} | {formatted_price:<12} | {p['quantity']:<5}"
        )
